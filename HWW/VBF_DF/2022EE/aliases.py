...
with open('/afs/cern.ch/user/s/squinto/private/work/PlotsConfigurationRun3/HWW/ggH_DF/2022EE/THU/NormTHU.json', 'r') as f:
    NormTHU = json.load(f)
...
aliases['fakeW'] = {
    ...
    'linesToProcess' : [f"ROOT.gInterpreter.ProcessLine('fake_rate_reader fr_reader = fake_rate_reader(\"cutBased_MediumID_tthMVA_Run3\", \"{muWP}\", \"nominal\", 2, \"std\", \"{fakerates}\", \"2022EE_v12_pt\");')"],
    ...
    for lep in ['Ele','Mu']:
        for variation in ['Up','Down']:
            aliases['fakeW'+stat+lep+variation] = {
                'linesToAdd'     : [f'#include "{macros}fake_rate_reader_class.cc"'],
                'linesToProcess' : [f"ROOT.gInterpreter.ProcessLine('fake_rate_reader fr_reader{stat}{lep}{variation} = fake_rate_reader(\"cutBased_MediumID_tthMVA_Run3\", \"{muWP}\", \"{stat}{lep}{variation}\", 2, \"std\", \"{fakerates}\", \"2022EE_v12_pt\");')"],
