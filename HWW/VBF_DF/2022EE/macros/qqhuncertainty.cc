...
public:
    ...
    QQHUncertainty(const std::string& name) {
        if      (name == "THU_qqH_Yield")   unc_index = 0;
        else if (name == "THU_qqH_PTH200")  unc_index = 1;
        else if (name == "THU_qqH_Mjj60")   unc_index = 2;
        else if (name == "THU_qqH_Mjj120")  unc_index = 3;
        else if (name == "THU_qqH_Mjj350")  unc_index = 4;
        else if (name == "THU_qqH_Mjj700")  unc_index = 5;
        else if (name == "THU_qqH_Mjj1000") unc_index = 6;
        else if (name == "THU_qqH_Mjj1500") unc_index = 7;
        else if (name == "THU_qqH_PTH25")   unc_index = 8;
        else if (name == "THU_qqH_JET01")   unc_index = 9;
        else {
            std::cerr << "QQHUncertainty: Unknown name " << name << std::endl;
        }
    }

    double operator()(int event_STXS, double Nsigma = 1.0) const {
        return vbf_uncert_stage_1_1(unc_index, event_STXS, Nsigma);
    }
};

class QQHEWKUncertainty {
public:
    bool with_imc_photon = false;

    QQHEWKUncertainty(bool include_photon = false) : with_imc_photon(include_photon) {}

    double operator()(int event_STXS, double Nsigma = 1.0) const {
        double weight = 1.0 + vbf_ew_correction_stage_1_1(event_STXS, with_imc_photon);
        return 1.0 + Nsigma * (weight - 1.0);
    }
};